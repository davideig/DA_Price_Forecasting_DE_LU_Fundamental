...
DEFAULT_WIND_VALUE_COLUMN = "Wind_Onshore_Model_MW"
...
ENERGY_ARENA_WIND_ONSHORE_CHALLENGE_IDS = {6, 12, 18}
...
def _validate_wind_submission_column(challenge_id: int, value_column: str) -> None:
    """Keep the known DE-LU wind challenges aligned with their onshore target."""
    if challenge_id in ENERGY_ARENA_WIND_ONSHORE_CHALLENGE_IDS and value_column != "Wind_Onshore_Model_MW":
        raise ValueError(
            "Energy Arena DE-LU wind challenges 6/12/18 are scored against "
            "ENTSO-E Wind Onshore (psrType B19). Submit Wind_Onshore_Model_MW, "
            f"not {value_column!r}."
        )
