...
def _expected_day_index(forecast_date: date, target_tz: str) -> pd.DatetimeIndex:
    """Canonical quarter-hourly market grid for one delivery day.

    Returns 96 units on a regular day and 92 or 100 units on the spring and
    autumn daylight-saving transition days, matching the actual market time
    units the Energy-Arena benchmark scores on (23-, 24-, or 25-hour days).
    """
    start = pd.Timestamp(forecast_date, tz=target_tz)
    end = pd.Timestamp(forecast_date + timedelta(days=1), tz=target_tz)
    return pd.date_range(start, end, freq="15min", inclusive="left")


def _align_forecast_day(
    work: pd.DataFrame, forecast_date: date, target_tz: str
) -> pd.DataFrame:
    """Select one delivery day on its canonical grid, handling DST-length days.

    Aligning by reindex rather than a label slice avoids the mixed-UTC-offset
    error that a ``loc`` range hits on transition days, and yields exactly the
    92, 96, or 100 units of that day.
    """
    expected_index = _expected_day_index(forecast_date, target_tz)
    present = expected_index.isin(work.index)
    if not present.any():
        raise ValueError(f"No predictions found for forecast date {forecast_date}.")
    if not present.all():
        raise ValueError(
            f"Forecast for {forecast_date} is missing {int((~present).sum())} of "
            f"the {len(expected_index)} market time units expected for {target_tz}; "
            f"daylight-saving transition days have 92 or 100 units rather than 96."
        )
    return work.reindex(expected_index)
...
def extract_point_predictions(
    ...
    day_df = _align_forecast_day(work, forecast_date, target_tz)[[value_column]].copy()
...
def extract_quantile_predictions(
    ...
    day_df = _align_forecast_day(work, forecast_date, target_tz)[list(quantile_columns)].copy()
...
def build_model_metadata(model_config: Any | None) -> dict[str, Any]:
    ...
    if isinstance(model_config, LearOperationalConfig):
        return {
            ...
            "price_model_type": model_config.price_model_type,
